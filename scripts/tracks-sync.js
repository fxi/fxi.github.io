...
const FEATURED_PATH = join(ROOT, "src", "content", "tracks", "featured.yaml");
...
async function getStravaToken() {
  ...
  throw new Error(
    "Could not obtain Strava access token — set STRAVA_REFRESH_TOKEN or STRAVA_ACCESS_TOKEN",
  );
}
...
async function stravaGet(path, token) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Strava GET ${path} → HTTP ${res.status}: ${body.slice(0, 300)}`,
    );
  }
  return res.json();
}
...
async function main() {
  ...
  catalogue = catalogue.map((entry) => {
    ...
    const newEntry = {
      ...
      elevation: hasElevation
        ? elevation
        : geometry
          ? downsample(geometry, ELEVATION_PTS).map((p) => Math.round(p[2]))
          : [],
    };
  ...
  if (migrated > 0)
    console.log(
      `  Migrated ${migrated} existing entry(ies): extracted elevation, removed geometry + map_url`,
    );
  ...
  console.log(
    `  To add: ${toAdd.length}, to remove: ${toDelete.length}, unchanged: ${catalogue.length - toDelete.length}`,
  );
  ...
  if (toAdd.length > 0) {
    ...
    for (const feat of toAdd) {
      ...
      try {
        const activity = await stravaGet(
        ...
        const fullPoints = latlng.map(([lat, lng], i) => [
          lng,
          lat,
          altitude[i] ?? 0,
        ]);
        ...
        const elevation = downsample(fullPoints, ELEVATION_PTS).map((p) =>
          Math.round(p[2]),
        );
        ...
        if ((activity.total_photo_count ?? 0) > 0) {
          ...
          for (let i = 0; i < toFetch.length; i++) {
            const p = toFetch[i];
            const photoUrl =
              p?.urls?.["2000"] ?? p?.urls?.[Object.keys(p?.urls ?? {})[0]];
        ...
        catalogue.push({
          ...
          date: (activity.start_date_local ?? activity.start_date ?? "").slice(
            0,
            10,
          ),
